...
def get_fw_version(source, target, env):
    ...
    for item in buildFlags.get("CPPDEFINES"): 
        if (type(item) is list):
            if (item[0]=="FTD_VER_MAJ"): FTD_VER_MAJ = item[1]
            if (item[0]=="FTD_VER_MIN"): FTD_VER_MIN = item[1]
            if (item[0]=="FTD_VER_REV"): FTD_VER_REV = item[1]
...
def bin_copy_rename(source, target, env):
    variant = str(target[0]).split(os.path.sep)[2] + '_v' + str(FTD_VER_MAJ) + '.' + str(FTD_VER_MIN) + '.' + str(FTD_VER_REV)
